...
func testUserToManyGroupsUsers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadGroupsUsers(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpGroupsUsers(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`ID`: `bigint`, `OrgID`: `bigint`, `UID`: `character varying`, `Name`: `character varying`, `Owner`: `boolean`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`}
	_           = bytes.MinRead
)
